pipeline {
   ...
   environment {
      ...
      ECR_REPOSITORY = '655244090840.dkr.ecr.us-east-1.amazonaws.com/code2cloud-ecr'
        ...
        stage('Container Scan') {
         steps {
               withCredentials([
                  ...
                  sh '''
                  twistcli images scan --address $PCC_CONSOLE_URL -u $PRISMA_ACCESS_KEY -p $PRISMA_SECRET_KEY --details $ECR_REPOSITORY:$CONTAINER_NAME
                  '''
               }
            }
        }
